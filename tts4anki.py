    ...
    with open(targetfilename,'w',encoding="utf-8") as targetf:
        with open(sourcefilename, 'r',encoding="utf-8") as sourcef:
            ...
            for input_csv_row in lines:
                ...
                tts_question=gTTS(text=q_and_a[0],lang=question_language,slow=False)
                question_mp3_file = str(prefix+'{:04d}'.format(row_number))+"_"+question_language+".mp3"
                print(q_and_a[0])
                tts_question.save(question_mp3_file)

                tts_answer=gTTS(text=q_and_a[1],lang=answer_language,slow=False)
                answer_mp3_file = str(prefix+'{:04d}'.format(row_number))+"_"+answer_language+".mp3"
                print(q_and_a[1])
                tts_answer.save(answer_mp3_file)

                targetf.write(q_and_a[0]+'[sound:'+question_mp3_file+'];'+q_and_a[1]+'[sound:'+answer_mp3_file+']\n')
    return(row_number)
...
def press(button):
    if button == "Process":
        ...
        if len(prefix) == 0:
            ...
            prefix = prefix + "_"
        pairs = mainproc(question_language,answer_language,input_filename,prefix)
        info = str(2*pairs) + " sound files are ready to import. Happy learning!"
